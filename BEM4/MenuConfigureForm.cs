...
namespace BEM4
{
    public partial class MenuConfigureForm : Form
    {
        ...
        private void MenuConfigureForm_Load(object sender, EventArgs e)
        {
            ...
            {
                mePath = MyDefine.myXET.userDAT + @"\Wifi.txt";
        ...
        private void MenuConfigureForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            else
            {
                if (DialogResult.Yes ==
                    ...
                    MessageBoxEX.Show("Network is being configured for the device. Confirm exit?", "Hint", MessageBoxButtons.YesNo, new string[] { "Yes", "No" }))
                    )
                {
                    timer.Stop();
                    if (server == null)
